...
from tdf import RawTdfFile
from crypto import create_local_key, create_legacy_local_key, decrypt_local
from qt import read_qt_byte_array, read_qt_int32
...
def decrypt_maps_tdf(settings_tdf: RawTdfFile, local_key):
    encrypted_stream = BytesIO(settings_tdf.encrypted_data)

    read_qt_byte_array(encrypted_stream)

    read_qt_byte_array(encrypted_stream)

    mapEncrypted = read_qt_byte_array(encrypted_stream)

    return decrypt_local(mapEncrypted, local_key)
